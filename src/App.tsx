...
import { Panel8 } from './components/panels/Panel8';
...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setCurrentPanel(Math.round(progress * 7));
    };
  ...
  useGSAP(() => {
    ...
    (window as any).scrollToPanel = (index: number) => {
      ...
      gsap.to(window, { scrollTo: (index / 7) * maxScroll, duration: 1, ease: "power2.inOut" });
    };
  ...
  return (
    <div ref={containerRef} className="bg-[#000] text-white selection:bg-[#fff] selection:text-black">
      ...
      <div className={`fixed inset-y-0 left-4 md:left-8 flex items-center z-50 pointer-events-none transition-opacity duration-300 ${currentPanel === 0 ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
        <button 
          onClick={() => {
            ...
            gsap.to(window, { scrollTo: (target / 7) * maxScroll, duration: 1, ease: "expo.inOut" });
          }}
      ...
      <div className={`fixed inset-y-0 right-4 md:right-8 flex items-center z-50 pointer-events-none transition-opacity duration-300 ${currentPanel >= 7 ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
        <button 
          onClick={() => {
            const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
            const target = Math.min(7, currentPanel + 1);
            gsap.to(window, { scrollTo: (target / 7) * maxScroll, duration: 1, ease: "expo.inOut" });
          }}
      ...
      <div ref={scrollWrapperRef} className="relative h-[100dvh] overflow-hidden flex z-10 pointer-events-none">
        <div ref={panelsRef} className="flex h-full w-[800vw] flex-nowrap items-center will-change-transform pointer-events-auto">
          ...
          <div className="w-screen h-full shrink-0 flex items-center justify-center relative"><Panel8 /></div>
        </div>
      </div>
    </div>
  );
